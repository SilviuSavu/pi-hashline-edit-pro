...
describe("chained edit anchors", () => {
  it("returns updated anchors in edit result for a single-line replace", async () => {
    await withTempFile("sample.ts", "alpha\nbeta\ngamma\n", async ({ cwd }) => {
      ...
      const editResult = await editTool.execute(
        "e1",
        { path: "sample.ts", remove_from: betaRef, remove_to: betaRef, replacement_lines: ["BETA"] },
        undefined,
        undefined,
        ctx,
      );
      ...
      const editResult2 = await editTool.execute(
        "e2",
        { path: "sample.ts", remove_from: freshRef, remove_to: freshRef, replacement_lines: ["BETA-CHAINED"] },
        undefined,
        undefined,
        ctx,
      );
  ...
  it("omits anchors when single-line replace expands beyond budget", async () => {
    ...
    await withTempFile("expand.ts", "before\ntarget\nafter\n", async ({ cwd }) => {
      ...
      const editResult = await editTool.execute(
        "e1",
        { path: "expand.ts", remove_from: targetRef, remove_to: targetRef, replacement_lines: newLines },
        undefined,
        undefined,
        ctx,
      );
  ...
  it("unchanged line anchors from original read remain valid after chained edits", async () => {
    await withTempFile("stale.ts", "alpha\nbeta\n", async ({ cwd }) => {
      ...
      await editTool.execute(
        "e1",
        { path: "stale.ts", remove_from: betaRef, remove_to: betaRef, replacement_lines: ["BETA"] },
        undefined,
        undefined,
        ctx,
      );
      await expect(
        editTool.execute(
          "e2-stale",
          { path: "stale.ts", remove_from: betaRef, remove_to: betaRef, replacement_lines: ["BETA-AGAIN"] },
          undefined,
          undefined,
          ctx,
        ),
      ...
      const alphaEdit = await editTool.execute(
        "e3",
        { path: "stale.ts", remove_from: alphaRef, remove_to: alphaRef, replacement_lines: ["ALPHA"] },
        undefined,
        undefined,
        ctx,
      );
  ...
  it("keeps untouched-line anchors valid after a reversed-range replace", async () => {
    await withTempFile("stable.ts", "alpha\nbeta\ngamma\ndelta\n", async ({ cwd }) => {
      ...
      const editResult = await editTool.execute(
        "e1",
        { path: "stable.ts", remove_from: gammaRef, remove_to: betaRef, replacement_lines: ["X"] },
        undefined,
        undefined,
        ctx,
      );
      ...
      const alphaEdit = await editTool.execute(
        "e2",
        { path: "stable.ts", remove_from: alphaRef, remove_to: alphaRef, replacement_lines: ["ALPHA"] },
        undefined,
        undefined,
        ctx,
      );
